...
router.post("/predict", async (req, res) => {
    ...
    // Split data for Transaction and Auth models
    const transactionData = {
        TransactionAmount: userData.TransactionAmount,
        OldBalanceOrig: userData.OldBalanceOrig,
        NewBalanceOrig: userData.NewBalanceOrig,
    ...
    const authData = {
        ...
        OS_BrowserInfo: userData.OS_BrowserInfo,
        PrivilegedAccess: userData.PrivilegedAccess
    };

    try {
        const transactionPrediction = await runPythonModel("ml/transaction_predict.py", transactionData);
        const authPrediction = await runPythonModel("ml/auth_predict.py", authData);

        res.json({
            transaction_fraud_risk: transactionPrediction,
            auth_fraud_risk: authPrediction
        });
    } catch (error) {
        console.error(error);
...
const runPythonModel = (scriptPath, inputData) => {
